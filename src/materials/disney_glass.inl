...
Spectrum eval_op::operator()(const DisneyGlass &bsdf) const {
    ...
    Real Ds = GTR2_aniso(dot(frame.n, half_vector), dot(frame.x, half_vector), dot(frame.y, half_vector), ax, ay);
    Real Gs = GGX_aniso(fabs(dot(dir_out, frame.n)), dot(dir_out, frame.x), dot(dir_out, frame.y), ax, ay) *
        GGX_aniso(fabs(dot(dir_in, frame.n)), dot(dir_in, frame.x), dot(dir_in, frame.y), ax, ay);


    if (reflect) {
        return base_color * (F * Ds * Gs) * fabs(dot(frame.n, dir_out));
    }
    else {
        ...
        return sqrt(base_color) * (eta_factor * (1 - F) * Ds * Gs * eta * eta * fabs(h_dot_out * h_dot_in)) * 4.0 * fabs(dot(frame.n, dir_out)) /
